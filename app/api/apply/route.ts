...
// Where guided-install applications are delivered. Overridable via env
// without a code change — same recipient as the contact form.
...
// .vercel.app — so we require that suffix. Anything wider would let arbitrary
// *.vercel.app projects (first-come, globally unique subdomains) POST here
// cross-origin on our Resend quota.
...
// The only tiers the apply form offers. Anything else is a hand-rolled POST.
const TIERS = ["Install", "Guided", "Partner"];
...
export async function POST(req: NextRequest) {
  ...
  const runs = String(body.runs ?? "").trim();
  ...
  const subject = `Wieman OS application — ${name}${company ? ` (${company})` : ""} — ${tier}`;
  const html = `
    <div style="font-family:system-ui,-apple-system,sans-serif;font-size:15px;line-height:1.6;color:#111">
      <h2 style="margin:0 0 18px;font-size:18px;font-weight:700">New guided-install application</h2>
      ...
      <p style="margin:0 0 8px"><strong>Tier:</strong> ${escapeHtml(tier)}</p>
      ${runs ? `<p style="margin:16px 0 0"><strong>What they run today:</strong><br>${escapeHtml(runs).replace(/\n/g, "<br>")}</p>` : ""}
      ${notes ? `<p style="margin:16px 0 0"><strong>Anything else:</strong><br>${escapeHtml(notes).replace(/\n/g, "<br>")}</p>` : ""}
      <hr style="border:none;border-top:1px solid #eee;margin:22px 0" />
      <p style="color:#888;font-size:13px;margin:0">Sent from the wiemansystems.com/wieman-os/apply form.</p>
    </div>`;
  const text = `New guided-install application

Name: ${name}
Email: ${email}${company ? `\nCompany: ${company}` : ""}
Tier: ${tier}${runs ? `\n\nWhat they run today:\n${runs}` : ""}${notes ? `\n\nAnything else:\n${notes}` : ""}

— wiemansystems.com/wieman-os/apply`;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: "Could not send your application. Please email us directly." },
        { status: 502 }
      );
    }
